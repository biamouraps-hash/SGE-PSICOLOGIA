...
const Layout = ({ children, user, onLogout }: any) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeSettings = onSnapshot(
      ...
      (error) => {
        console.warn("App settings snapshot failed (using default offline settings):", error);
        setAppSettings({
          name: "SGE Psicologia",
          logoUrl: "https://images.weserv.nl/?url=i.imgur.com/NR6kaz6.png"
        });
  ...
  useEffect(() => {
    ...
    const isCentral =
      activeUnit === "Administração Central" || activeUnit === "Sede";
    let reportsQuery;

    const isSuperAdmin = isSuperUser(user);
    const allowedUnits = (user?.units || []).map((u: string) =>
      u.trim().toUpperCase(),
    );
    const userId = user?.id || user?.uid;

    const baseReportsQuery = query(
    ...
    if (isSuperAdmin) {
      if (!isCentral) {
        reportsQuery = query(
          baseReportsQuery,
          where("unit", "==", activeUnit.trim().toUpperCase()),
        );
      } else {
      ...
      if (
        !isCentral &&
        allowedUnits.includes(activeUnit.trim().toUpperCase())
      ) {
        reportsQuery = query(
          baseReportsQuery,
          where("unit", "==", activeUnit.trim().toUpperCase()),
      ...
      async (error) => {
        ...
        try {
          ...
          setUnreadReportsCount(
            items?.filter((r: any) => !r.analyzed).length || 0,
          );
        } catch (fallbackErr) {
    ...
    const unsubscribeRequests = onSnapshot(
      ...
      async (error) => {
        console.warn(
          "Pending requests listener failed, trying fallback:",
          error,
        );
        try {
    ...
    const unsubscribeAppointments = onSnapshot(
      finalAppointmentsQuery,
      (snapshot) => {
        const todayItems = snapshot.docs.filter((doc) => {
          const data = doc.data();
          return (
            data.date?.startsWith(todayStr) &&
        ...
        const tomorrowItems = snapshot.docs.filter((doc) => {
          const data = doc.data();
          return (
            data.date?.startsWith(tomorrowStr) &&
      ...
      async (error) => {
        ...
        try {
          ...
          const todayItems =
            items?.filter(
              (r: any) =>
                r.date?.startsWith(todayStr) && r.status !== "cancelled",
            ) || [];
          const tomorrowItems =
            items?.filter(
              (r: any) =>
                r.date?.startsWith(tomorrowStr) && r.status === "confirmed",
            ) || [];
  ...
  const menuCategories = [
      ...
      items: [
        {
          ...
          path: "/documentos?type=group_attendance",
          permission: "documents",
        },
        {
          icon: <GraduationCap size={18} />,
          label: "Ações Pedagógicas",
          path: "/documentos?type=pedagogical_participation",
          permission: "documents",
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          icon: <Inbox size={18} />,
          label: "Caixinha do Acolhimento",
          path: "/gestao-denuncias",
          permission: "psychological_listening",
        },
      ],
    },
    {
  ...
  const filteredCategories = menuCategories
    .map((cat) => ({
      ...
      items: cat.items.filter((item) => {
        ...
        if (
          user?.role === "aee" &&
          (item.path === "/atendimento-aee" ||
            item.path.startsWith("/documentos") ||
            item.path === "/alunos" ||
            item.path === "/escolas")
        )
          return true;
      ...
      >
        <div className="flex flex-col h-full py-6 px-6">
          <div className="mb-8 flex flex-col items-center gap-2 shrink-0">
            ...
            <p className="text-[9px] font-black text-pedagogic-blue uppercase tracking-[0.3em] mt-2 text-center">
              {appSettings.name}
            </p>
            ...
            {filteredCategories.map((cat) => (
              <div key={cat.id} className="space-y-1.5">
                ...
                >
                  ...
                  <div
                    className={`transition-transform duration-300 ${expandedCategories.includes(cat.id) ? "rotate-180" : ""}`}
                  >
                    <ChevronDown
                      size={12}
                    ...
                    {cat.items.map((item) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span
                            className={`transition-colors ${isPathActive(item.path) ? "text-white" : "text-pedagogic-blue/40"}`}
                          >
                            {React.cloneElement(
            ...
            <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-center gap-3 group relative">
              ...
              >
                ...
                <div className="flex-1 overflow-hidden">
                  <p className="text-[11px] font-black text-slate-800 truncate leading-none group-hover:text-pedagogic-blue transition-colors">
                    {user?.name}
                  </p>
                  <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1 opacity-70 truncate">
                    {user?.role}
                  </p>
                </div>
              </Link>
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (err.code === "auth/network-request-failed") {
        setError(
          "ERRO DE REDE: Falha ao conectar ao serviço de autenticação do Firebase. Isso pode ser causado por um bloqueador de anúncios, firewall corporativo ou se o domínio '" +
            window.location.hostname +
            "' não estiver autorizado nas configurações do Firebase console.",
        );
      } else if (err.code === "auth/unauthorized-domain") {
        setError(
          "DOMÍNIO NÃO AUTORIZADO: Adicione '" +
            window.location.hostname +
            "' em Autenticação > Configurações > Domínios Autorizados no Console do Firebase.",
        );
      } else {
        setError(
        ...
        <div className="bg-white rounded-[3rem] shadow-2xl p-12 border border-white relative overflow-hidden">
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div className="relative flex items-center py-4">
              ...
              <span className="flex-shrink mx-4 text-[9px] font-black text-slate-300 uppercase tracking-[0.3em]">
                Ou use E-mail
              </span>
              <div className="flex-grow border-t border-slate-100"></div>
            </div>
...
const Register = ({ onLogin }: any) => {
  ...
  const [phone, setPhone] = useState(
    () => localStorage.getItem("temp_lead_phone") || "",
  );
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === "auth/network-request-failed") {
        ...
        setError(
          "ESTE DOMÍNIO NÃO ESTÁ AUTORIZADO NO FIREBASE. No console do Firebase, adicione '" +
            window.location.hostname +
            "' em Autenticação > Configurações > Domínios Autorizados.",
        );
      } else if (err.code === "auth/popup-blocked") {
        setError(
          "O POP-UP FOI BLOQUEADO. Por favor, permita pop-ups para este site para continuar.",
        );
      } else {
        setError(
      ...
      <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-xl p-12 relative overflow-hidden">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 font-medium">
            Inicie seu teste grátis de 7 dias ou 10 atendimentos agora mesmo.
          </p>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100/50 text-center backdrop-blur-sm">
              <p className="text-[10px] font-black text-pedagogic-blue uppercase tracking-widest mb-4">
              ...
              <p className="mt-4 text-[9px] text-slate-400 font-bold uppercase tracking-tighter">
                ...
                <span className="text-pedagogic-blue underline cursor-pointer">
                  termos de uso
                </span>
                .
              </p>
        ...
        <p className="text-center mt-10 text-[10px] font-black text-slate-400 uppercase tracking-widest">
          Ao prosseguir, você concorda com nossos{" "}
          <span className="text-pedagogic-blue cursor-pointer">
            Termos de Uso
          </span>
          .
        </p>
...
const Dashboard = ({ user }: { user: any }) => {
  ...
  useEffect(() => {
    try {
      const localSchools = localStorage.getItem("sge_local_schools");
      const localStudents = localStorage.getItem("sge_local_students");
      const hasSchools = localSchools && JSON.parse(localSchools).length > 0;
      const hasStudents = localStudents && JSON.parse(localStudents).length > 0;
      setHasLocalData(!!(hasSchools || hasStudents));
    } catch (e) {
  ...
  const handleMigrate = async () => {
    setMigrating(true);
    try {
      const res = await api.utils.migrateLocalDataToFirebase();
      setHasLocalData(false);
      alert(`Sincronização concluída com sucesso! ${res.totalMigrated} registros foram enviados para o Firebase Firestore.`);
      window.location.reload(); 
    } catch (err) {
      console.error("Migration failed:", err);
      alert("Erro ao migrar dados: " + (err instanceof Error ? err.message : String(err)));
    } finally {
      setMigrating(false);
    }
  };
  ...
  const handleCreateTestData = async () => {
    ...
    try {
      await api.utils.createTestSampleData(user.id || user.uid || "test_user_id", user.name || "SGE Psicólogo");
      alert(`Sucesso absoluto!\n\nDados criados diretamente no Firestore:\n1. Escola de Teste: "SESI Escola de Teste"\n2. Aluno de Teste: "Arthur Dent (Aluno de Teste)"\n3. Atendimento de Teste: agendado para hoje às 15:00.`);
      window.location.reload();
    } catch (err) {
      console.error("Test data seeding failed:", err);
      alert("Erro ao cadastrar dados de teste no Firebase: " + (err instanceof Error ? err.message : String(err)));
    } finally {
    ...
    const loadStats = async () => {
      try {
        ...
        if (!isSuperAdmin) {
          ...
          filters.allowedUnits = (user?.units || []).map((u: string) =>
            u.trim().toUpperCase(),
          );
        }
        ...
        const [
          ...
          usageService.getSubscriptionStatus(
            activeUnit,
            !isSuperAdmin ? user.id || user.uid : undefined,
          ),
        ]);
        ...
        const filterByUnit = (item: any) => {
          ...
          const userAllowedUnits = (user?.units || []).map((u: string) =>
            u.trim().toUpperCase(),
          );

          const sFromId = (schoolsData || []).find(
            (s: any) => s.id === (item.schoolId || item.unitId),
          ) as any;
          const itemUnits = [
            String(item.schoolUnit || "")
              .trim()
              .toUpperCase(),
            String(item.unit || "")
              .trim()
          ...
          const isGlobalUnit =
            selectedUnit === "ADMINISTRAÇÃO CENTRAL" || selectedUnit === "SEDE";
            ...
            return itemUnits.some((itemUnit) =>
              userAllowedUnits.some((allowed: string) => {
                const cleanAllowed = allowed
                  .replace(
                    /^(SESI|UNIDADE|ESCOLA|CENTRO|DEPARTAMENTO)\s+/gi,
                    "",
                  )
                  .trim();
                const cleanItem = itemUnit
                  .replace(
                    /^(SESI|UNIDADE|ESCOLA|CENTRO|DEPARTAMENTO)\s+/gi,
                    "",
                  )
                ...
                return (
                  itemUnit === allowed ||
                  (cleanAllowed !== "" && cleanAllowed === cleanItem)
                );
              }),
            );
          }
          ...
          const cleanSelected = selectedUnit
            .replace(/^(SESI|UNIDADE|ESCOLA|CENTRO|DEPARTAMENTO)\s+/gi, "")
            .trim();
          ...
          return itemUnits.some((val) => {
            ...
            if (
              val === selectedUnit ||
              (cleanVal !== "" && cleanVal === cleanSelected)
            )
              return true;
            if (cleanSelected !== "" && cleanVal !== "") {
              return (
                cleanVal.includes(cleanSelected) ||
                cleanSelected.includes(cleanVal)
              );
            }
        ...
        const filteredPendingReqs = (pendingReqsData || []).filter(
          filterByUnit,
        );
        const filteredStudents = (studentsData || []).filter(filterByUnit);
        const filteredAppointmentsMonth = (appointmentsMonthData || []).filter(
          filterByUnit,
        );
        const filteredDocumentsMonth = (documentsMonthData || []).filter(
          filterByUnit,
        );
        ...
        setStats({
          ...
          appointments:
            filteredAppointmentsMonth.length + filteredDocumentsMonth.length,
        ...
        if (isTrial) {
          console.log(
            `[Trial Debug] Usage: ${usageStatus?.current}/${usageStatus?.limit} (${usageStatus?.percentage}%)`,
          );
        }
        ...
        setReports(
          filteredReports
            .filter(
              (r: any) =>
                (r.aiAnalysis?.level === "CRÍTICO" ||
                  r.aiAnalysis?.level === "MODERADO" ||
                  r.aiAnalysis?.level === "PENDENTE" ||
                  !r.aiAnalysis) &&
                r.analyzed !== true,
            )
            .sort(
        ...
        filteredStudents.forEach((student: any) => {
          const schoolRef = schoolsData.find(
            (s: any) => s.id === student.schoolId,
          );
          const schoolName =
            (schoolRef as any)?.name ||
            student.schoolUnit ||
            student.unit ||
            "Sem Unidade";
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4 bg-white/10 p-4 rounded-3xl backdrop-blur-sm border border-white/10">
              <div className="text-center px-4">
                ...
                <div className="bg-white/20 w-32 h-2 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (trialDaysLeft / 7) * 100)}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="text-center px-4">
                ...
                <div className="bg-white/20 w-32 h-2 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-pedagogic-teal transition-all duration-1000"
                    style={{
                      width: `${Math.min(100, ((usage?.current || 0) / (usage?.limit || 10)) * 100)}%`,
                    }}
                  />
                </div>
              </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-1">
                Status da Unidade:{" "}
                <span className="text-pedagogic-blue">{activeUnit}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 bg-white p-2 rounded-3xl border border-slate-100 shadow-sm">
            {usage && (
              <div className="flex items-center gap-4 pr-4 border-r border-slate-100">
                <div className="flex flex-col items-end">
                  ...
                  <p
                    className={`text-xs font-black mt-1 ${usage.percentage > 90 ? "text-red-500" : usage.percentage > 70 ? "text-amber-500" : "text-pedagogic-teal"}`}
                  >
            ...
            <div className="flex -space-x-2">
              <div className="w-8 h-8 rounded-full bg-pedagogic-blue border-2 border-white flex items-center justify-center text-[10px] font-black text-white">
                1
              </div>
              <div className="w-8 h-8 rounded-full bg-pedagogic-teal border-2 border-white flex items-center justify-center text-[10px] font-black text-white">
                2
              </div>
              <div className="w-8 h-8 rounded-full bg-pedagogic-amber border-2 border-white flex items-center justify-center text-[10px] font-black text-white">
                3
              </div>
            </div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest pr-4 border-r border-slate-100">
              Equipe On
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          >
            ...
            <div className="flex items-baseline gap-2">
              <p className="text-6xl font-black text-pedagogic-blue tabular-nums tracking-tighter">
                {stats.students}
              </p>
          ...
          >
            ...
            <p className="text-6xl font-black text-pedagogic-teal tabular-nums tracking-tighter">
              {stats.appointments}
            </p>
            <div className="mt-6 flex items-center gap-2">
              <div className="flex gap-1">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="w-2 h-2 rounded-full bg-pedagogic-teal/30"
                  ></div>
                ))}
              </div>
          ...
          >
            ...
            <p
              className={`text-xs font-black uppercase tracking-[0.2em] mb-4 ${stats.criticalReports > 0 ? "text-white/80" : "text-slate-400"}`}
            >
              Alertas Críticos
            </p>
            <div className="flex items-baseline gap-3">
              ...
              {stats.criticalReports > 0 && (
                <Zap size={24} className="animate-pulse text-white mb-2" />
              )}
            </div>
            <div className="mt-6">
              <div
                className={`h-1.5 w-full rounded-full overflow-hidden ${stats.criticalReports > 0 ? "bg-white/20" : "bg-slate-50"}`}
              >
                <div
                  className={`h-full rounded-full ${stats.criticalReports > 0 ? "bg-white w-full" : "bg-pedagogic-rose w-0"}`}
                ></div>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-black text-purple-600 tabular-nums tracking-tighter">
                {stats.appointments}
              </p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-0.5">
                Alunos Ativos por Unidade Escolar
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 bg-slate-900 rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl group flex flex-col justify-between">
          ...
          <div className="relative z-10">
            <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center mb-6">
              <Zap
                className="text-pedagogic-blue fill-pedagogic-blue"
                size={28}
              />
            </div>
            <h3 className="text-3xl font-black tracking-tight mb-4 leading-tight">
              Missão do Dia:
              <br />
              <span className="text-pedagogic-blue">Acolhimento</span>
            </h3>
            <p className="text-slate-400 font-medium mb-8">
              Confira seus atendimentos e pendências antes de iniciar a jornada.
            </p>
          </div>
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl hover:bg-white/10 transition-colors cursor-pointer border border-white/5 group/quest">
              ...
              <div className="flex-1">
                ...
                <p className="font-bold text-white tracking-tight">
                  {stats.todayAppointments} Agendamentos
                </p>
              </div>
              <ArrowRight
                size={18}
                className="text-slate-600 group-hover/quest:translate-x-1 group-hover/quest:text-white transition-all"
              />
            </div>
            ...
            <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl hover:bg-white/10 transition-colors cursor-pointer border border-white/5 group/quest">
              ...
              <div className="flex-1">
                ...
                <p className="font-bold text-white tracking-tight">
                  {stats.criticalReports} Casos Críticos
                </p>
              </div>
              <ArrowRight
                size={18}
        ...
        <div className="lg:col-span-8 space-y-8">
          {stats.criticalReports > 0 && (
            <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                ...
                <Link
                  to="/gestao-denuncias"
                  className="text-[10px] font-black uppercase tracking-widest text-pedagogic-blue hover:underline"
                >
                  Ver Todos
                </Link>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {reports
                  ...
                  .map((report) => (
                    <div
                      key={report.id}
                      className="p-6 bg-slate-50/50 rounded-3xl border border-slate-100 hover:border-pedagogic-rose/30 transition-all group"
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="px-3 py-1 bg-white border border-slate-100 rounded-full">
                          <p
                            className={`text-[10px] font-black uppercase tracking-widest ${report.aiAnalysis?.level === "CRÍTICO" ? "text-pedagogic-rose" : "text-slate-400"}`}
                          >
                            {schools.find((s) => s.id === report.schoolId)
                              ?.name || "Unidade Geral"}
                          </p>
                        </div>
                        <p className="text-[10px] font-bold text-slate-400">
                          {report.timestamp
                            ? format(new Date(report.timestamp), "dd/MM/yyyy", {
                                locale: ptBR,
                              })
                            : "Agora"}
                        </p>
          ...
          <div className="grid grid-cols-1 gap-8">
            <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col">
              ...
              <div className="space-y-4 flex-1">
                {upcomingAppointments.length === 0 ? (
                  ...
                  upcomingAppointments.map((app) => (
                    <div
                      key={app.id}
                      className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100/50"
                    >
              ...
              <Link
                to="/atendimentos"
                className="mt-6 text-[10px] font-black text-center uppercase tracking-widest text-pedagogic-blue hover:underline"
              >
                Agenda Completa
              </Link>
            </div>
          ...
          {stats.criticalReports === 0 && upcomingAppointments.length === 0 && (
            <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center">
              ...
              <p className="text-slate-400 font-medium max-w-sm">
                Você não possui alertas ou pendências urgentes no momento.
              </p>
            </div>
          )}
        </div>
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (!firebaseUser) {
        // Safe check for our bypassed administrator before removing session!
        const savedUser = localStorage.getItem("user");
        if (savedUser) {
          try {
            const parsed = JSON.parse(savedUser);
            if (parsed && (parsed.id === "local_super_admin" || parsed.email === "administrador@sgepsicologia.com")) {
              setUser(parsed);
              setAuthReady(true);
              setLoading(false);
              return;
      ...
      if (firebaseUser.isAnonymous) {
        // Safe check for our bypassed administrator before setting anonymous!
        const savedUser = localStorage.getItem("user");
        if (savedUser) {
          try {
            const parsed = JSON.parse(savedUser);
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          if (isSuperUser(userData)) {
            ...
            adminPerms.forEach((p) => {
              if (!userData.permissions.includes(p))
                userData.permissions.push(p);
            });
          }
          ...
          if (
            firebaseUser.email === "maykon.euro@gmail.com" ||
            firebaseUser.email === "administrador@exemplo.com" ||
            firebaseUser.email === "administrador@sgepsicologia.com"
          ) {
            ...
            try {
              await setDoc(doc(db, "users", firebaseUser.uid), superAdmin);
              console.log("Successfully created super admin profile in Firestore!");
            } catch (fsErr) {
              console.warn("Could not write super admin profile to Firestore (using memory fallback):", fsErr);
            }
            const syncedUser = { id: firebaseUser.uid, ...superAdmin };
            setUser(syncedUser);
      ...
      <UnitProvider user={user}>
        <Routes>
          ...
          <Route
            ...
            element={
              isSuperUser(user) ||
              user?.permissions?.includes("psychological_listening") ? (
          ...
          <Route
            path="/importar"
            element={
              user?.permissions?.includes("import_students") ||
              isSuperUser(user) ? (
                <Layout user={user} onLogout={handleLogout}>
          ...
          <Route
            path="/atendimento-aee"
            element={
              user?.permissions?.includes("documents") ||
              user?.permissions?.includes("aee_records") ||
              user?.role === "aee" ||
              isSuperUser(user) ? (
                <Layout user={user} onLogout={handleLogout}>
          ...
          <Route
            path="/escolas"
            element={
              user?.permissions?.includes("schools") ||
              user?.role === "aee" ||
              isSuperUser(user) ? (
                <Layout user={user} onLogout={handleLogout}>
                  <Schools user={user} />
          ...
          <Route
            path="/solicitacoes"
            element={
              user?.permissions?.includes("scheduling_requests") ||
              isSuperUser(user) ? (
                <Layout user={user} onLogout={handleLogout}>
