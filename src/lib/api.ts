...
const LOCAL_STORAGE_PREFIX = "sge_local_";

const isLocalBypass = () => {
  // We no longer bypass Firebase for standard admins/users. This ensures every register is written 
  // directly to the live Firestore database and appears in the Firebase web console.
  return false;
};
...
const LSDb = {
  ...
  list: (name: string, filters?: any) => {
    let items = LSDb.getCollection(name);
    if (filters) {
      if (filters.status) {
      ...
      if (filters.unit && filters.unit !== 'Administração Central' && filters.unit !== 'Sede') {
        ...
        items = items.filter(item => {
          const itemUnit = item.unit ? String(item.unit).toUpperCase() : '';
          return itemUnit.includes(u) || u.includes(itemUnit);
        });
      }
      if (filters.schoolId) {
        items = items.filter(item => item.schoolId === filters.schoolId);
      }
    }
    return items;
  },
  ...
  get: (name: string, id: string) => {
    const items = LSDb.getCollection(name);
    return items.find(item => item.id === id) || null;
  },

  create: (name: string, data: any, id?: string) => {
    const items = LSDb.getCollection(name);
    const newId = id || `local_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    const newItem = {
      ...data,
      id: newId,
    ...
    items.push(newItem);
    LSDb.saveCollection(name, items);
    return newItem;
  },
  ...
  update: (name: string, id: string, data: any) => {
    ...
    return LSDb.create(name, data, id);
  },
...
const initMockData = () => {
  // 1. schools
  if (LSDb.getCollection("schools").length === 0) {
    const defaultSchools = [
      {
        id: "school_1",
        name: "SESI Vila Leopoldina",
        unit: "ADMINISTRAÇÃO CENTRAL",
        address: "Rua Carlos Weber, 835 - Vila Leopoldina, São Paulo - SP",
        phoneNumber: "(11) 3833-1099",
        directorName: "Silvia Maria",
        directorEmail: "silvia.maria@sesisp.org.br",
        active: true,
        code: "SESI-VL1",
        ownerId: "local_super_admin",
        createdAt: new Date().toISOString()
      },
      {
        id: "school_2",
        name: "SESI Santo André",
        unit: "ADMINISTRAÇÃO CENTRAL",
        address: "Praça Armando de Salles Oliveira, 1 - Centro, Santo André - SP",
        phoneNumber: "(11) 4997-1000",
        directorName: "Marcos Antonio",
        directorEmail: "marcos.antonio@sesisp.org.br",
        active: true,
        code: "SESI-SA2",
        ownerId: "local_super_admin",
        createdAt: new Date().toISOString()
    ...
    LSDb.saveCollection("schools", defaultSchools);
  }

  // 2. students
  if (LSDb.getCollection("students").length === 0) {
    const defaultStudents = [
      {
        id: "student_1",
        name: "Ana Julia Santos",
        registrationCode: "RM38491",
        birthDate: "2012-05-14",
        grade: "6º Ano B",
        schoolId: "school_1",
        schoolName: "SESI Vila Leopoldina",
        unit: "ADMINISTRAÇÃO CENTRAL",
        guardianName: "Marta Santos",
      ...
      },
      {
        id: "student_3",
        name: "Luiza Moreira",
        registrationCode: "RM29384",
        birthDate: "2010-02-28",
        grade: "8º Ano C",
        schoolId: "school_2",
        schoolName: "SESI Santo André",
        unit: "ADMINISTRAÇÃO CENTRAL",
        guardianName: "Renata Moreira",
        guardianPhone: "(11) 91234-5678",
        status: "active",
        createdAt: new Date().toISOString()
      }
    ];
    LSDb.saveCollection("students", defaultStudents);
  }

  // 3. appointments
  if (LSDb.getCollection("appointments").length === 0) {
    const defaultAppointments = [
      {
        id: "appointment_1",
        studentId: "student_1",
        studentName: "Ana Julia Santos",
        studentGrade: "6º Ano B",
        schoolId: "school_1",
        schoolName: "SESI Vila Leopoldina",
        unit: "ADMINISTRAÇÃO CENTRAL",
        date: new Date().toISOString().split('T')[0],
        time: "14:00",
        type: "Escuta Psicológica",
        professionalId: "local_super_admin",
        professionalName: "Super Administrador",
        status: "confirmed",
        observations: "Primeiro acolhimento. Aluna demonstra ansiedade em períodos avaliativos.",
        createdAt: new Date().toISOString()
      }
    ];
    LSDb.saveCollection("appointments", defaultAppointments);
  }

  // 4. appointmentTypes
  if (LSDb.getCollection("appointment_types").length === 0) {
    LSDb.saveCollection("appointment_types", [
      { id: "type_1", name: "Escuta Psicológica", color: "#3b82f6" },
      { id: "type_2", name: "Orientação Pedagógica", color: "#10b981" },
      { id: "type_3", name: "Atendimento de AEE", color: "#f59e0b" },
      { id: "type_4", name: "Reunião de Pais", color: "#ef4444" }
    ]);
  }

  // 5. categories
  if (LSDb.getCollection("categories").length === 0) {
    LSDb.saveCollection("categories", [
      { id: "cat_1", name: "Dificuldade de Aprendizagem" },
      { id: "cat_2", name: "Ansiedade e Estresse" },
      { id: "cat_3", name: "Problemas de Comportamento" },
      { id: "cat_4", name: "Conflitos Familiares" }
    ]);
  }

  // 6. SaaS plans
  if (LSDb.getCollection("plans").length === 0) {
    LSDb.saveCollection("plans", [
      { id: "trial", name: "Período de Testes", price: "0,00", maxAppointments: 10, maxSchools: 1 },
      { id: "basic", name: "Plano Básico", price: "R$ 79,90/mês", maxAppointments: 40, maxSchools: 2 },
      { id: "professional", name: "Plano Profissional", price: "R$ 199,90/mês", maxAppointments: 120, maxSchools: 5 },
      { id: "premium", name: "Plano Premium", price: "R$ 499,90/mês", maxAppointments: 500, maxSchools: 15 }
    ]);
  }
};
...
const makeFailoverHandler = (targetObj: any, collectionName: string) => {
  const handler: any = {};
  
  Object.keys(targetObj).forEach(key => {
    const originalMethod = targetObj[key];
    if (typeof originalMethod === 'function') {
      handler[key] = async (...args: any[]) => {
        if (isLocalBypass()) {
          return executeLSDb(collectionName, key, args);
        }
        
        try {
          return await originalMethod(...args);
        } catch (error: any) {
          console.warn(`[LSDb Failover] Firebase failed for '${collectionName}.${key}'. Falling back to local storage.`, error);
          return executeLSDb(collectionName, key, args);
        }
      };
    } else {
      handler[key] = targetObj[key];
    }
  });
...
const executeLSDb = (collectionName: string, methodName: string, args: any[]): any => {
  initMockData();
  
  switch (methodName) {
    case 'list':
      return LSDb.list(collectionName, args[0]);
    case 'get':
      return LSDb.get(collectionName, args[0]);
    case 'create':
      return LSDb.create(collectionName, args[0], args[1]);
    case 'update':
      return LSDb.update(collectionName, args[0], args[1]);
    case 'delete':
      return LSDb.delete(collectionName, args[0]);
    
    case 'findByEmail': {
      const email = args[0];
      const items = LSDb.getCollection('users');
      return items.find(u => u.email === email) || null;
    }
    case 'initializeTrial': {
      const user = args[0];
      const extraData = args[1];
      const items = LSDb.getCollection('users');
      let existing = items.find(u => u.email === user.email);
      ...
      const trialData = {
        ...
        email: user.email || 'profissional@sgepsicologia.com',
      ...
      items.push(trialData);
      LSDb.saveCollection('users', items);
      return trialData;
    }
    
    case 'get': {
      if (collectionName === 'appSettings') {
        const items = LSDb.getCollection('app_settings');
        return items.find(item => item.id === 'global') || {
          name: "SGE Psicologia",
    ...
    case 'update': {
      if (collectionName === 'appSettings') {
        const items = LSDb.getCollection('app_settings');
        const index = items.findIndex(item => item.id === 'global');
        const updated = { ...args[0], id: 'global', updatedAt: new Date().toISOString() };
        if (index !== -1) {
          items[index] = { ...items[index], ...updated };
        } else {
          items.push(updated);
        }
        LSDb.saveCollection('app_settings', items);
        return updated;
      }
      return LSDb.update(collectionName, args[0], args[1]);
    }
    ...
    default:
      console.warn(`LSDb method '${methodName}' fallback for '${collectionName}'.`);
      if (methodName === 'list' || methodName === 'query') return [];
      return null;
  }
};
...
const baseApi = {
  ...
  seed: async () => {
    ...
    return baseApi.fetch("/seed", { method: "POST" });
  },
  ...
  utils: {
    ...
    migrateLocalDataToFirebase: async () => {
      const collectionsToMigrate = [
        { ls: 'schools', fs: 'schools' },
        { ls: 'students', fs: 'students' },
        { ls: 'appointments', fs: 'appointments' },
        { ls: 'appointment_types', fs: 'appointment_types' },
        { ls: 'categories', fs: 'categories' },
        { ls: 'letterheads', fs: 'letterheads' },
        { ls: 'document_layouts', fs: 'document_layouts' },
        { ls: 'documents', fs: 'documents' },
        { ls: 'scheduling_requests', fs: 'scheduling_requests' },
        { ls: 'app_settings', fs: 'app_settings' }
      ];
      ...
      for (const item of collectionsToMigrate) {
        try {
          const raw = localStorage.getItem(LOCAL_STORAGE_PREFIX + item.ls);
          if (raw) {
            ...
            if (Array.isArray(data) && data.length > 0) {
              for (const record of data) {
                ...
                if (recordId) {
                  ...
                  const docRef = doc(db, item.fs, recordId);
                  
                  // Map any standard date strings back to timestamp or keep them if string is needed
                  const cleanedRecord: any = { ...rest };
                  for (const key of Object.keys(cleanedRecord)) {
            ...
            } else if (data && typeof data === 'object' && item.ls === 'app_settings') {
              // Handle single object collections like global app settings
          ...
          console.error(`Error migrating collection ${item.ls}:`, e);
          errors++;
        }
      }
      return { totalMigrated, errors };
    },
    createTestSampleData: async (uid: string, professionalName: string) => {
      const schoolId = `school_tst_${Date.now()}`;
      const studentId = `student_tst_${Date.now()}`;
      const appointmentId = `appt_tst_${Date.now()}`;
      
      await setDoc(doc(db, "schools", schoolId), {
        name: "SESI Escola de Teste",
        unit: "ADMINISTRAÇÃO CENTRAL",
        address: "Av. Paulista, 1313 - Bela Vista, São Paulo - SP",
        phoneNumber: "(11) 3214-5678",
        directorName: "Diretoria SGE",
        directorEmail: "direcao.teste@sesisp.org.br",
        active: true,
        code: "SESI-TST-01",
        ownerId: uid,
        createdAt: new Date().toISOString()
      ...
      await setDoc(doc(db, "students", studentId), {
        name: "Arthur Dent (Aluno de Teste)",
        registrationCode: "RM99999",
        birthDate: "2012-10-10",
        grade: "7º Ano A",
        schoolId: schoolId,
        schoolName: "SESI Escola de Teste",
        unit: "ADMINISTRAÇÃO CENTRAL",
        guardianName: "Mãe do Aluno de Teste",
      ...
      await setDoc(doc(db, "appointments", appointmentId), {
        studentId: studentId,
        studentName: "Arthur Dent (Aluno de Teste)",
        studentGrade: "7º Ano A",
        schoolId: schoolId,
        schoolName: "SESI Escola de Teste",
        unit: "ADMINISTRAÇÃO CENTRAL",
        date: new Date().toISOString().split('T')[0],
        ...
        type: "Escuta Psicológica",
        professionalId: uid,
        professionalName: professionalName || "SGE Psicólogo",
        status: "confirmed",
        observations: "Atendimento de teste gerado com sucesso pelo botão de validação rápida.",
        createdAt: new Date().toISOString()
      });

      return { schoolId, studentId, appointmentId };
    }
  }
};

export const api = {
  users: makeFailoverHandler(baseApi.users, 'users'),
  students: makeFailoverHandler(baseApi.students, 'students'),
  appointments: makeFailoverHandler(baseApi.appointments, 'appointments'),
  appointmentTypes: makeFailoverHandler(baseApi.appointmentTypes, 'appointment_types'),
  categories: makeFailoverHandler(baseApi.categories, 'categories'),
  documents: makeFailoverHandler(baseApi.documents, 'documents'),
  schools: makeFailoverHandler(baseApi.schools, 'schools'),
  schedulingRequests: makeFailoverHandler(baseApi.schedulingRequests, 'scheduling_requests'),
  letterheads: makeFailoverHandler(baseApi.letterheads, 'letterheads'),
  leads: makeFailoverHandler(baseApi.leads, 'leads'),
  documentLayouts: makeFailoverHandler(baseApi.documentLayouts, 'document_layouts'),
  anonymousReports: makeFailoverHandler(baseApi.anonymousReports, 'anonymous_reports'),
  appSettings: makeFailoverHandler(baseApi.appSettings, 'appSettings'),
  plans: makeFailoverHandler(baseApi.plans, 'plans'),
  subscriptions: makeFailoverHandler(baseApi.subscriptions, 'subscriptions'),
  usageLogs: makeFailoverHandler(baseApi.usageLogs, 'usage_logs'),
  seed: baseApi.seed,
  utils: baseApi.utils,
  fetch: baseApi.fetch
};
